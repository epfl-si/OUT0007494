...
damage <-
    binlogs %>%
    filter({
        kind <- classify.binlog(.)
        kind == "Line 3665 damage" | kind == "Repair"
    }) %>%
...
damage %>% group_by(photo_ext) %>% tally
```

Le dommage ligne 3665 de `/opt/dinfo/lib/perl/Cadi/People.pm` est indiscernable dans les binlogs des gens qui revertent manuellement à leur photo CAMIPRO en ligne 3543.
